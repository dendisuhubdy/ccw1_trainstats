...
parser.add_argument('--setup', type=str, default='custom',
                    help='Not custom means that hyperparameters may be overwriten to apply chosen setup (FD|ND|WH)')
...
parser.add_argument('--save_dir', type=str,  default='/data/lisatmp3/suhubdyd/models/',
...
parser.add_argument('--replacing_prob', type=float, default=0,
                    help='probability for input words to be replaced (replacing_prob = 0 means no replacement)')
...
if args.setup == 'FD':
    ...
    if args.kappa <= 0:
        args.kappa = 0.3 #0.25 is also okay
elif args.setup == 'WH':
    if args.kappa <= 0:
        args.kappa = 0.25
    if args.replacing_prob == 0:
        print("Probability for input words replacing (replacing_prob) changed to 0.5 since it was zero")
        args.replacing_prob = 0.5
else:
    print("Warning! Custom setup is used, you may try to check FD|ND|WH options before")
...
def train():
    ...
    while i < train_data.size(0) - 1 - 1:
        ...
        model.train()
        output, new_hidden, rnn_h, dropped_rnn_h, emb, Wh = model(data, hidden, return_h=True)
       ...
       # 
        ...
        if args.kappa > 0:
            ...
            # draw-mask is default
            #
            kappa_output, _, _, _, _, _ = model(data, hidden, return_h=True, draw_mask=draw_mask, emb=None)
            
            loss = loss + criterion(kappa_output.view(-1, ntokens), targets)
            loss = loss/2
            # loss metioned in meeting
            # main thing
            # this is the l2 kappa 
